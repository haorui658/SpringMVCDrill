...
<div class="container" style="padding: 15px">
    <div class="row">
        ...
        <a href="/http/userCase/add/${id}">
            <button id="query" class="col-md-2 btn btn-primary">新增</button>
        </a>
        <a href="/http/index">
            <button id="query" class="col-md-2 btn btn-primary">返回</button>
        </a>
    </div>
...
<script>
    $(document).ready(function () {
        ...
        $("#main tbody").on("click", "#sendBtn", function () {
            //获取行
            var row = $("table#main tr").index($(this).closest("tr"));
            //获取某列（从0列开始计数）的值
            var id = $("table#main").find("tr").eq(row).find("td").eq(0).text();

                $.post("/http/userCase/request/" + id,
                        function (data, status) {
                            if (status == 'success') {
                                table.ajax.reload();
                                alert(data);
        ...
        $("#main tbody").on("click", "#editBtn", function () {
            ...
            var row = $("table#main tr").index($(this).closest("tr"));
            var id = $("table#main").find("tr").eq(row).find("td").eq(0).text();
            window.location.href = "/http/userCase/edit/" + id;
        });
    });
